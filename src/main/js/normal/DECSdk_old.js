...
var componentsArray = ["alerts","appmanager","calendar","commerce","connectivity","identity","media","navigation","notification","policy","sa",
                       "search","settings","sms","va","vehicleinfo"];
...
var DecCommon = function() {
			...
			if (this.isLeaf()) {
				...
				if (this.isArray) {
					...
					for (var counter = 0; counter < itemsArray.length; counter++) {
						...
						if (found) {
							arrayVal[arrayIndex] = JSON
									.stringify(itemsArray[counter]);
							arrayIndex++;
						}
					}
		...
		if (attrName === null) {
			...
			try {
				...
				if (this.isLeaf() && !(this.isArray || this.isMap)) {
					...
					executeSubscriptionCallbackForParentChild(mNodeLevel, valueToSend);
					console.log("Notifying Systems");
					notifySystems(data);
					...
					if (this.isArray) {
						...
						console.log("Executing Subscription callback method");
						executeSubscriptionCallbackForParentChild(this.nodeLevel, valueToSend);
						console.log("Notifying Systems");
						notifySystems(data);
						...
						console.log("Executing Subscription callback method");
						executeSubscriptionCallbackForParentChild(this.nodeLevel, options);
						console.log("Notifying Systems");
						notifySystems(data);
	...
	this.delete = function(options) {
		...
		// Send the message to Span. The receiveMessage in the SpanMessageBroker or mbbridge would 
		// delete the items on receiving the notifications.
		
		/*if (filter) {
			// Delete called with options
			var deletedItem = handleDeleteWithOptions(this.nodeLevel, options, this.isLeaf(), this.isArray, this.isMap); 
			console.log("Deleted Item ::: ", deletedItem);
		} else {
			// Delete called without options
			var deletedItem = handleDeleteWithNoOptions(this.nodeLevel, this.isLeaf(), this.isArray, this.isMap);
			console.log("Deleted Item ::: ", deletedItem);
		}*/
...
function notifySpan(data) {
	...
	console.log("Host received to open the socket is " + properties['host']);
	console.log("Port received to open the socket is " + properties['port']);
...
function executeSubscriptionCallbackAfterSet(handle, value) {
	...
	if (subscriptionArray == null || subscriptionArray.length == 0) {
		...
		for (var counter = 0; counter < subscriptionArray.length; counter++) {
			...
			subCallback(subValue, EventType.GET);
		}
	}
 ...
 * @param options
 */
function executeSubscriptionCallbackForParentChild(node, options) {
    ...
    for (var i = childNodes.length - 1; i >= 0; i--) {
    	...
    	if (i == childNodes.length - 1) {// navigation.destination.display
    		executeSubscriptionCallbackAfterSet(node, options);
    		executeNode = node;
    	} else {
    		var attrKey = executeNode.substring(executeNode.lastIndexOf(".")+1);
    		var valueToSend = {};
			valueToSend[attrKey] = newValue;
    		//nodeValue = executeNode.substring(executeNode.lastIndexOf(".")+1) + ":" + JSON.stringify(options);
    		...
    		executeSubscriptionCallbackAfterSet(executeNode, valueToSend); 
